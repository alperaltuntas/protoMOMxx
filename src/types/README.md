...
This directory is a layer below both `src/initialization` and the future parameterizations, because both need these types: the initialization code computes their contents, the dynamics reads them, and `src/core`'s stepper calls the dynamics. Keeping the types below everyone keeps the per-directory libraries one-way: framework <- types <- initialization <- core.
...
MOM6 keeps most of these files in its `src/core` (e.g., `MOM_grid.F90`). The move down is deliberate, so as to keep the library graph acyclic, and the MOM6 file names are kept.
